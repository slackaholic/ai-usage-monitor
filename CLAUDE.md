# CLAUDE.md — AI Usage Monitor

Orientation for an AI agent picking up this repo. Read this, then `README.md`
(user-facing feature tour) and the relevant spec in `docs/superpowers/specs/`.
...
and the memory note `memory/reference_usage_data_sources.md`. In short:
...
- **Workflow:** features go through superpowers — `brainstorming` → spec in
  `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` → `writing-plans` →
  plan in `docs/superpowers/plans/` → implement via
  `subagent-driven-development` (or `executing-plans`) with TDD. The user
  approves specs and plans before implementation. Bugs go through
...
- **Verify before commit:** `npm test` (must stay green — 36 tests) and
  ...
  launched in the agent environment** — GUI verification is the user's step;
  say so rather than claiming visual confirmation.
- **Environment:** Windows. Use the **Bash tool (Git Bash)** for POSIX shell /
  superpowers scripts; PowerShell is also available. Primary dir:
...
- **`memory/`** (machine-local, not in git) — durable cross-session facts
  (`reference_usage_data_sources.md`, login/import gotchas). The
  `.superpowers/sdd/` ledger is also local scratch — neither travels via clone.
